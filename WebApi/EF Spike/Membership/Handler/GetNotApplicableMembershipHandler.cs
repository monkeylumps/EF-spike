...
namespace EF_Spike.Membership.Handler
{
    public class GetNotApplicableMembershipHandler : IRequestHandler<GetNotApplicableMembership, List<Model.Membership>>
    {
        ...
        {
            var membership = await context.TblMembership.Include(x => x.TblMembershipDetails).Include(x => x.TblMembershipAverageAgeBasis).Where(x => x.Psrnumber == request.Psr && x.EndDate == null && x.EndEventReference == null && x.TblMembershipAverageAgeBasis.Any(y => y.MembershipAverageAgeBasis == 3)).ToListAsync(cancellationToken);
