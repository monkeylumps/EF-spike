...
namespace EF_Spike.Membership.Handler
{
    public class PostMembershipHandler : IRequestHandler<PostMembership, Model.Membership>
    {
        ...
        private readonly IRequestHandler<CreateEvent, int> eventRequest;

        public PostMembershipHandler(RegistryContext context, IRequestHandler<CreateEvent, int> eventRequest)
        {
        ...
        {
            var eventId = await eventRequest.Handle(new CreateEvent { Event = request.Event }, cancellationToken);
